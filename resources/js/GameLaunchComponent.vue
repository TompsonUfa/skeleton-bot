<template>
    <div class="game-launch d-flex justify-content-center flex-column">
        <h6 class="mt-1">Game launch</h6>
...
export default {
    ...
    methods: {
        getData(data = {}) {
            ...
            axios.post('/charts/game-launch', data)
                .then(res => {
                    this.avgSum = res.data.avg;
                    this.datacollection = res.data
                    this.isLoading = false
                    let time = new Date().getTime() - start_time;
                    console.log(time + 'ms - Chart game-launch');
                })
        },
        makeActive(target) {
            let actEl = document.querySelector('.game-launch .act1')
            if (actEl) {
